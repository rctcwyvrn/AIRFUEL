...
func _draw() -> void:
	...
	for t: Dictionary in charge_threats:
		...
		_draw_bearing_arc(center, RING_RADIUS, t.angle, half_width, color,
				lerpf(3.0, 9.0, progress))
...
func _draw_bearing_arc(center: Vector2, radius: float, bearing: float,
		half_width: float, color: Color, thickness: float) -> void:
	# Bearing 0 = ahead = screen-up; draw_arc's 0 is +x, so shift by -PI/2.
	draw_arc(center, radius, bearing - PI / 2.0 - half_width,
			bearing - PI / 2.0 + half_width, ARC_POINTS, color, thickness, true)
